...
def save_mstmp(ms_tensor,path='./tmp_ms.npy'):
    arr=ms_tensor.asnumpy().astype(np.float32)
    np.save(path,arr)

def save_torchtmp(tensor,path='/home/zyz/code/GMAms/tmp_torch.npy'):
    arr=tensor.detach().cpu().numpy().astype(np.float32)
    np.save(path,arr)

def l1_dis(ms_path='./tmp_ms.npy',torch_path='./tmp_torch.npy'):
    arr_ms=np.load(ms_path)
    arr_torch=np.load(torch_path)
    return np.mean(np.abs(arr_ms-arr_torch))
