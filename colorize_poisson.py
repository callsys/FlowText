...
class FontColor(object):
    ...
    def sample_from_data(self, bg_mat):
        ...
        color_dis = -1
        t = 0
        while color_dis < params['color']['color_dis']:
            col1 = self.sample_normal(data_col[:3],data_col[3:6])
            col2 = self.sample_normal(data_col[6:9],data_col[9:12])
            color_dis = np.abs(col1.astype(np.float32)-col2.astype(np.float32)).sum()
            t = t+1
            if t>30:
                raise ValueError
...
class Colorize4(object):
    ...
    def __init__(self, model_dir='data'):#, im_path):
        ...
        # color_file = 'models/colors_new.cp'
        # color_file = 'models/colors_ic13.cp'
        color_file = params['color']['source']
        # color_file = 'models/colors_coco.cp'
        # color_file = 'models/colors_joint.cp'
        self.font_color = FontColor(col_file=osp.join(model_dir,color_file))
    ...
    def color_border_determinstic(self, col_text, col_bg, seed):
        """
        Decide on a color for the border:
            - could be the same as text-color but lower/higher 'VALUE' component.
            - could be the same as bg-color but lower/higher 'VALUE'.
        ...
        if choice==0:
            # increase/decrease saturation:
            if 'cbd_1' not in seed:
                seed['cbd_1'] = get_sample(col_text[0])
            col_text[0] = seed['cbd_1']
            # col_text[0] = get_sample(col_text[0]) # saturation
            col_text = np.squeeze(cv.cvtColor(col_text[None,None,:],cv.COLOR_HSV2RGB))
        elif choice==1:
            # get the complementary color to text:
        ...
        # now change the VALUE channel:
        col_text = np.squeeze(cv.cvtColor(col_text[None,None,:],cv.COLOR_RGB2HSV))
        if 'cbd_2' not in seed:
            seed['cbd_2'] = get_sample(col_text[2])
        col_text[2] = seed['cbd_2']
        # col_text[2] = get_sample(col_text[2]) # value
        return np.squeeze(cv.cvtColor(col_text[None,None,:],cv.COLOR_HSV2RGB)),seed

    def process_determinstic(self, text_arr, bg_arr, min_h, seed=None):
        """
        text_arr : one alpha mask : nxm, uint8
        bg_arr   : background image: nxmx3, uint8
        min_h    : height of the smallest character (px)
        ...
        if seed is None:
            ...
            s1 = np.random.rand(2)
            s2 = np.random.choice(3)
            s3 = np.random.randn(5)
            s4 = np.random.choice([1, 3, 5, 7])
            seed = {'fg_col':fg_col,'bg_col':bg_col,'s1':s1,'s2':s2,'s3':s3,'s4':s4}
        else:
            fg_col = seed['fg_col']
            bg_col = seed['bg_col']
            s1,s2,s3,s4 = seed['s1'],seed['s2'],seed['s3'],seed['s4']
            l_text = Layer(alpha=text_arr, color=fg_col)
        # bg_col = np.mean(np.mean(bg_arr, axis=0), axis=0)
        l_bg = Layer(alpha=255 * np.ones_like(text_arr, 'uint8'), color=bg_col)

        # l_text.alpha = l_text.alpha * np.clip(0.88 + 0.1 * s3[0], 0.72, 1.0)
        a, b, c = params['color']['merge_range']
        l_text.alpha = l_text.alpha * np.clip(b + 0.1 * s3[0], a, c)
        layers = [l_text]
        blends = []

        # add border:
        if s1[0] < self.p_border:
            if min_h <= 15:
                bsz = 1
            elif 15 < min_h < 30:
                bsz = 3
                ...
                bsz = 5
            border_a = self.border(l_text.alpha, size=bsz)
            out, seed = self.color_border_determinstic(l_text.color, l_bg.color, seed)
            l_border = Layer(border_a, out)
            layers.append(l_border)
            blends.append('normal')

        # add shadow:
        if s1[1] < self.p_drop_shadow:
            # shadow gaussian size:
            if min_h <= 15:
                bsz = 1
            elif 15 < min_h < 30:
        ...
        if l_out is None:
            ...
            # just do a normal blend:
            layers[-1] = l_bg
            return self.merge_down(layers, blends).color, seed
    ...
    #         s1 = np.random.rand()
    #         s2 = np.random.rand()
    #         s3 = np.random.choice(3)
    #         s4 = np.random.randn()
    #         s5 = np.random.randn()
    #         s6 = np.random.randn()
    #         s7 = np.random.randn()
    #         s8 = np.random.randn()
    #         seed = {'fg_col':fg_col,'bg_col':bg_col,'s1':s1,'s2':s2,'s3':s3,'s4':s4,'s5':s5,'s6':s6,'s7':s7,'s8':s8}
    #     else:
    #         fg_col = seed['fg_col']
    #         bg_col = seed['bg_col']
    #         s1,s2,s3,s4,s5,s6,s7,s8 = seed['s1'],seed['s2'],seed['s3'],seed['s4'],seed['s5'],seed['s6'],seed['s7'],seed['s8']
    #         l_text = Layer(alpha=text_arr, color=fg_col)
    #     # bg_col = np.mean(np.mean(bg_arr, axis=0), axis=0)
    #     l_bg = Layer(alpha=255 * np.ones_like(text_arr, 'uint8'), color=bg_col)
    #
    #     l_text.alpha = l_text.alpha * np.clip(0.88 + 0.1 * s4, 0.72, 1.0)
    #     layers = [l_text]
    #     blends = []
    #
    #     # add border:
    ...
    #             bsz = 5
    #         border_a = self.border(l_text.alpha, size=bsz)
    #         l_border = Layer(border_a, self.color_border_determinstic(l_text.color, l_bg.color, s3))
    #         layers.append(l_border)
    #         blends.append('normal')
    #
    #     # add shadow:
    #     if s2 < self.p_drop_shadow:
    #         # shadow gaussian size:
    #         if min_h <= 15:
    #             bsz = 1
    #         elif 15 < min_h < 30:
    ...
    #
    #         # shadow angle:
    #         theta = np.pi / 4 * np.random.choice([1, 3, 5, 7]) + 0.5 * s5
    #
    #         # shadow shift:
    #         if min_h <= 15:
    #             shift = 2
    ...
    # def batch_color(self, bg_arrs, text_arrs, hs, place_order=None, pad=20):
    #     """
    #     Return colorized text image.
    #
    #     text_arr : list of (n x m) numpy text alpha mask (unit8).
    ...
    #
    #     out_bg_arrs = []
    #     for bg_arr, text_arr in zip(bg_arrs, text_arrs):
    #         # try:
    #         text_arr = [text_arr]
    #         flag = False
    #         bg_arr = bg_arr.copy()
    ...
    #             ## this is the minimum x and y coordinates of text:
    #             loc = np.where(text_arr[i])
    #             if len(np.array(loc)[-1])==0:
    #                 out_bg_arrs.append(bg_arr)
    #                 continue
    #             try:
    #                 lx, ly = np.min(loc[0]), np.min(loc[1])
    #                 mx, my = np.max(loc[0]), np.max(loc[1])
    ...
    #                 bg = bg_arr[l[0]:l[0] + w, l[1]:l[1] + h, :]
    #
    #                 rdr0,seed = self.process_determinstic(text_patch, bg, hs[i], seed = seed)
    #                 rendered.append(rdr0)
    #
    #                 bg_arr[l[0]:l[0] + w, l[1]:l[1] + h, :] = rdr0  # rendered[-1]
    #
    ...
    def batch_color(self, bg_arrs, text_arrs, hs, place_order=None, pad=20):
        ...
        for bg_arr, text_arr in zip(bg_arrs, text_arrs):
            ...
            try:
                tmp_loc = np.array(np.where(text_arr[0]))
                if len(np.array(tmp_loc)[-1]) == 0:
                    out_bg_arrs.append(bg_arr)
                    continue
    ...
    def process2(self, text_arr, bg_arr, min_h, seed=0):
        """
        text_arr : one alpha mask : nxm, uint8
        bg_arr   : background image: nxmx3, uint8
        min_h    : height of the smallest character (px)
    ...
    def batch_color2(self, bg_arrs, text_arrs, hs, place_order=None, pad=20):
        """
        Return colorized text image.
        ...
        for bg_arr, text_arr in zip(bg_arrs, text_arrs):
            try:
                ...
                for i in place_order[::-1]:
                    ...
                    rdr0 = self.process2(text_patch, bg, hs[i], seed=seed)
                    rendered.append(rdr0)
