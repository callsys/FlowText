...
class RenderFont(object):
    ...
    def __init__(self, data_dir='data'):
        ...
        #                0.0 : 'LINE',
        #                1.0 : 'PARA'}
        # self.p_text = {0.7: 'WORD',
        #                0.25: 'LINE',
        #                0.05: 'PARA'}
        # self.p_text = {0.7: 'WORD',
        #                0.25: 'LINE',
        #                0.05: 'PARA'}
        self.p_text = params['text']['p_text']
        ...
        # source = 'newsgroup/ic15.txt'
        self.text_source = TextSource(min_nchar=self.min_nchar,
                                      fn=osp.join(data_dir,source))
        ...
        # self.font_state.fonts = [el for el in self.font_state.fonts if freetype.Font(el).name in self.font_state.font_model.keys()]
...
class FontState(object):
    ...
    def __init__(self, data_dir='data'):
        ...
        self.FONT_LIST2 = osp.join(data_dir, 'fonts/cusfontlist.txt')
        self.fonts = [os.path.join(data_dir,'fonts',f.strip()) for f in open(self.FONT_LIST)]
        self.fonts.extend([os.path.join(data_dir,'fonts',f.strip()) for f in open(self.FONT_LIST2)])
    ...
    def sample2(self):
        ...
        Samples from the font state distribution
        """
        font_cls = self.fonts_cls[int(np.random.randint(0, len(self.fonts_cls)))]
        sub_fonts = [el for el in self.fonts if font_cls in el]
        font = sub_fonts[int(np.random.randint(0, len(sub_fonts)))]
        return {
            'font': font,
            'size': self.size[1]*np.random.randn() + self.size[0],
            'underline': np.random.rand() < self.underline,
